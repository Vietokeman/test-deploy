# Script tự động setup Railway deployment
# Chạy script này để chuẩn bị project cho Railway deployment
...
if (-Not (Test-Path "src/Trippio.Api/Trippio.Api.csproj")) {
    Write-Host "❌ Error: Please run this script from the root directory (where Trippio.BE.sln is)" -ForegroundColor Red
...
Write-Host "📦 Step 1: Installing Npgsql packages..." -ForegroundColor Yellow
Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Failed to install Npgsql for Trippio.Data" -ForegroundColor Red
    Set-Location "../.."
    exit 1
}

Write-Host "  ✓ Npgsql installed for Trippio.Data" -ForegroundColor Green
Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Failed to install Npgsql for Trippio.Api" -ForegroundColor Red
    Set-Location "../.."
    exit 1
}

Write-Host "  ✓ Npgsql installed for Trippio.Api" -ForegroundColor Green
Set-Location "../.."
Write-Host ""
...
if (-Not $allFilesExist) {
    Write-Host ""
    Write-Host "❌ Some deployment files are missing. Please ensure all files are created." -ForegroundColor Red
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "⚠️  Warning: Git not initialized or not a git repository" -ForegroundColor Yellow
...
if ($commit -eq "y" -or $commit -eq "Y") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host ""
        Write-Host "🚀 You can now deploy on Railway!" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "ℹ️  Skipping git commit. Don't forget to commit and push manually!" -ForegroundColor Yellow
}
